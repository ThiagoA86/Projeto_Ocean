...
from app.libs.formularios import Form_Conexao
...
def insert():
     ...
     if request.method == "POST" and formulario.validate():
        database = formulario.database.data
        if database == '':
...
def update(id):
    ...
    formulario = Form_Conexao()
    if request.method == 'POST' and formulario.validate():
        new_database = formulario.database.data
        ...
        if new_database == '':
            ...
            try:
                ...
                Connection.update_conn(driver, nomeconexao, host, porta, new_database, username, password, id)
            except Exception as e:
